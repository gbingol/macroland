...
class pnlOutputOptions ( wx.CollapsiblePane ):
	...
	def __init__( self, parent:wx.Window, bgcolor = None, ShowPrettify = True):
		...
		#selection section
		sbSzrTargetLoc = wx.StaticBoxSizer( wx.StaticBox( self.GetPane(), label="Target Location" ))

		self.m_radioSelection = wx.RadioButton(sbSzrTargetLoc.GetStaticBox(), label="Selection")
		
		self.m_txtSelRange = GridTextCtrl(sbSzrTargetLoc.GetStaticBox())
		self.m_txtSelRange.Enable(False) #radiobox is selected, so disable it

		self.m_radioNewWS = wx.RadioButton( sbSzrTargetLoc.GetStaticBox(), label="New Sheet")
		self.m_radioNewWS.SetValue( True ) #by default new worksheet
		
		fgSzrLoc = wx.FlexGridSizer( 0, 2, 10, 0 )
		fgSzrLoc.AddGrowableCol( 1 )
		fgSzrLoc.SetFlexibleDirection( wx.BOTH )
		fgSzrLoc.SetNonFlexibleGrowMode( wx.FLEX_GROWMODE_SPECIFIED )
		fgSzrLoc.Add( self.m_radioSelection, 0, wx.ALL, 5 )
		fgSzrLoc.Add( self.m_txtSelRange, 0, wx.ALL|wx.EXPAND, 5 )
		fgSzrLoc.Add( self.m_radioNewWS, 0, wx.ALL, 5 )

		sbSzrTargetLoc.Add( fgSzrLoc, 1, wx.EXPAND, 5 )
		...
		self.m_radioSelection.Bind( wx.EVT_RADIOBUTTON, self.__OnRadio )
		self.m_radioNewWS.Bind( wx.EVT_RADIOBUTTON, self.__OnRadio )



	def __OnRadio( self, event:wx.CommandEvent ):
		self.m_txtSelRange.Enable(event.GetEventObject() == self.m_radioSelection)
		event.Skip()
	...
	def Get(self)->tuple[Worksheet|None, int, int]:
		...
		if self.m_radioNewWS.GetValue(): #new worksheet
			...
			SelRange = Range(txt=self.m_txtSelRange.GetValue())
	...
	def Prettify(self)->bool:
		return self.m_ChkPrettify.GetValue()
