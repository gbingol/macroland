...
static int ws_mp_ass_subscript(PyObject* self, PyObject* key, PyObject* rhs)
{
    ...
    if (IsSubTypeDict(rhs))
    {
        ...
        while (PyDict_Next(Dict, &pos, &DictKey, &DictValue))
        {
            ...
            {
                IF_PYERR_RETDEF(IsExactTypeString(DictValue) == false, PyExc_TypeError, "color values must be string type, \"R G B\"", -1);
...
static PyObject* wb_bindFunction(Python::Workbook* self, PyObject* args)
{
    auto SelfWB = (Python::Workbook*)self;

    size_t NArgs = PyTuple_GET_SIZE(args);

    if (NArgs < 2)
    {
        PyErr_SetString(PyExc_ValueError, "At least 2 parameters must be provided");
        return nullptr;
    }

    PyObject* EventNameObj = PyTuple_GetItem(args, 0);

    if (!PyUnicode_Check(EventNameObj))
    {
        PyErr_SetString(PyExc_ValueError, "event type must be a string.");
        return NULL;
    }
    ...
    try
    {
        ...
        SelfWB->ptrObj->BindPythonFunction(EventName, CallbackFunc);
    }
    catch (std::exception& e)
    {
        PyErr_SetString(PyExc_RuntimeError, e.what());
...
static PyObject* wb_UnbindFunction(Python::Workbook* self, PyObject* args)
{
    auto SelfWB = (Python::Workbook*)self;
    ...
    try
    {
        SelfWB->ptrObj->UnbindPythonFunction(EventName, FuncObj);
    }
    CATCHRUNTIMEEXCEPTION_RET();
...
static PyMethodDef PyWorkbook_methods[] =
{
    { "bind",
    (PyCFunction)wb_bindFunction,
    METH_VARARGS,
    "callbacks a function" },

    { "unbind",
    (PyCFunction)wb_UnbindFunction,
    METH_VARARGS,
    "unbinds the bound function" },
...
static int Workbook_init(Python::Workbook* self, PyObject* args, PyObject* kwargs)
{
    self->ptrObj = glbWorkbook;
    return 0;
}


static void wb_dealloc(Python::Workbook* self)
{
...
static PyObject* wb_mp_subscript(PyObject* self, PyObject* args)
{
    auto SelfWB = (Python::Workbook*)self;

    if (IsSubTypeTuple(args)) {
        PyErr_SetString(PyExc_ValueError, "Only single param of type int allowed");
        return nullptr;
    }

    int Pos = -1;

    if (IsSubTypeLong(args))
        Pos = PyLong_AsLong(args);


    CHECKNONNEGATIVE_RET(Pos, "index must be >=0");

    if (Pos >= SelfWB->ptrObj->size())
    {
        PyErr_SetString(PyExc_ValueError, "index exceeds number of available worksheets");
        return nullptr;
    }

    auto WS = (ICELL::CWorksheet*)SelfWB->ptrObj->GetWorksheet(Pos);
...
static PyObject* wb_tp_str(PyObject* self)
{
    auto WB = (Python::Workbook*)self;

    wxString str = "";

    PyObject* StrObj = PyUnicode_FromString(str.mb_str(wxConvUTF8));
...
static Py_ssize_t wb_sequence_len(PyObject* obj)
{
    auto WB = (Python::Workbook*)obj;
    return WB->ptrObj->size();
}



static PySequenceMethods wb_sequence_methods = { (lenfunc)wb_sequence_len };


static PyMappingMethods wb_MappingMethods = { };


PyTypeObject PythonWorkbook_Type = { PyVarObject_HEAD_INIT(NULL, 0) "Workbook" };





int PyInit_Workbook(PyObject* Module)
{
    wb_MappingMethods.mp_subscript = wb_mp_subscript;

    PythonWorkbook_Type.tp_new = PyType_GenericNew;
    PythonWorkbook_Type.tp_basicsize = sizeof(Python::Workbook);

    PythonWorkbook_Type.tp_dealloc = (destructor)wb_dealloc;
    PythonWorkbook_Type.tp_flags = Py_TPFLAGS_DEFAULT;
    PythonWorkbook_Type.tp_doc = "The global workbook";

    PythonWorkbook_Type.tp_methods = PyWorkbook_methods;
    PythonWorkbook_Type.tp_init = (initproc)Workbook_init;
    PythonWorkbook_Type.tp_str = (reprfunc)wb_tp_str;

    PythonWorkbook_Type.tp_as_sequence = &wb_sequence_methods;


    PythonWorkbook_Type.tp_as_mapping = &wb_MappingMethods;

    if (PyType_Ready(&PythonWorkbook_Type) < 0)
        return -1;




    Py_INCREF(&PythonWorkbook_Type);

    if (PyModule_AddObject(Module, "Workbook", (PyObject*)&PythonWorkbook_Type) < 0)
    {
        Py_DECREF(&PythonWorkbook_Type);
        return -1;
    }
