...
namespace grid
{
	...
	std::wstring Cell::ToHTMLElement_TD() const
	{
		std::wstringstream HTML;

		HTML << "<td ";
		HTML << "style = \"background-color:" << m_Format.GetBackgroundColor().GetAsString(wxC2S_HTML_SYNTAX).ToStdWstring() << ";";
		HTML << "color:" << m_Format.GetTextColor().GetAsString().ToStdWstring() << ";";
		HTML << "font-family:" << m_Format.GetFont().GetFaceName() << "; font-size:" << m_Format.GetFont().GetPointSize() << "pt;";

		if (m_Format.GetFont().GetUnderlined())
			HTML << "text-decoration: underline" << ";";

		if (m_Format.GetHAlign() == wxALIGN_LEFT)
			HTML << "text-align: left;";
		else if (m_Format.GetHAlign() == wxALIGN_CENTRE)
			HTML << "text-align: center;";
		else if (m_Format.GetHAlign() == wxALIGN_RIGHT)
			HTML << "text-align: right;";
		else
			HTML << "text-align: general" << ";";


		if (m_Format.GetVAlign() == wxALIGN_BOTTOM)
			HTML << "vertical-align: bottom;";
		else if (m_Format.GetVAlign() == wxALIGN_CENTRE)
			HTML << "vertical-align: middle;";
		else if (m_Format.GetVAlign() == wxALIGN_TOP)
			HTML << "vertical-align: top;";

		//to protect Excel's border, otherwise it looks white (cell borders are lost)
		HTML << "border:none; mso-background-source:auto; mso-pattern:auto; ";
		HTML << "\">"; //end of style


		if (m_Format.GetFont().GetWeight() == wxFONTWEIGHT_BOLD)
			HTML << "<strong>";
		if (m_Format.GetFont().GetStyle() == wxFONTSTYLE_ITALIC)
			HTML << "<em>";
