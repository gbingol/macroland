...
namespace script
{
	...
	public:
		...
		TreeItemData(
			...
			TreeItemData::TYPE type) : m_Path{ path }, m_Type{ type }
		{
			m_LineStart = 0;
			m_LineEnd = 0;
		}
		...
		void SetLineStart(int linest) { m_LineStart = linest; }
		auto GetLineStart() const { return m_LineStart; }

		void SetLineEnd(int lineend) { m_LineEnd = lineend; }
		auto GetLineEnd() const { return m_LineEnd; }
	...
	{
	public:
		DLLSCRIPT pnlPathBrowser(
			wxWindow* parent, 
			const std::list<std::filesystem::path>& PathList);
